...
const Menu = {
	...
	"Grinder": {
		price: 2625,
		emoji: '🏃',
		items: ["Chips","Chips","Chips","Chips","Chips","Lucky Ramen","Lucky Ramen","Lucky Ramen","Lucky Ramen","Lucky Ramen"],
        isSelected: false
	},
		// Individual Items:
        "Individual Items:": {
    ...
    const INDIVIDUAL_ITEMS = ["Lucky Ramen","Chips","Mac N Cheese","Coffee", "Iced Coffee","Pizza","Panini","Pancakes","Wings"];
...
function getEmptyOrder() {
	let buffer = [];
	buffer.push('<img src="images/SplitSidesLogo.png" width="45%">');
	buffer.push("");
	...
	Object.keys(Menu).forEach(item => {
		if (Menu[item].header) {
			...
			table += "<td><center><button class=\"btn\" title='Add 1x " + item + "' onClick='add(\"" + item + "\")'><strong>" + icon + item + "</strong></button><br />" +
				...
				"<i class=\"fa fa-edit\" aria-hidden=\"true\" title='Manually edit " + item + " quantity' onClick='editQuantity(\"" + item + "\")'></i> " +
